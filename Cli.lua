...
redis:select(Config.RedisIndex)
...
local var = false
for v,user in pairs(Config.SUDO_ID) do
...
if tonumber(user_id) == tonumber(Config.TD_ID) or Owner or Mod or Vip then
return true
else
return false
end
...
local url  = https.request('https://api.telegram.org/bot'..Config.token..'/getchatmember?chat_id='..Config.Channel..'&user_id='..msg.sender_user_id)
if res ~= 200 then
end
...
if redis:get("AddFilterPro:"..msg.chat_id) then
if TD_B0T:match('^([Oo][Kk])$') then
sendText(msg.chat_id, msg.id, '⇜ عملیات لغو شد !', 'md')
redis:del("AddFilterPro:"..msg.chat_id)
elseif TD_B0T:match('(.*)') then
local word = TD_B0T:match('(.*)')
redis:sadd('Filters:'..msg.chat_id,word)
sendText(msg.chat_id, msg.id, '⇜ کلمه `'..word..'` به لیست فیلتر اضافه شد\nاگر نمی خواهید ادامه بدهید با دستور "انجام شد" به عملیات پایان بدهید', 'md')
end
end
...
if status == "kickuser" then
 if tonumber(msg.sender_user_id) == tonumber(Config.TD_ID)  then
return true
end
...
if msg.sender_user_id and is_GlobalyBan(msg.sender_user_id) and not Config.TD_ID then
...
sendText(msg.chat_id, msg.id,text,'md')
sendText(Config.Sudoid, 0,textlogs,'html')
end
end
...
Left(chat_id,Config.TD_ID, "Left")
end 
if matches == 'panel gp' or matches == 'پنل گروه ها' then
...
get(Config.BotHelper, msg.chat_id, 0, 0, "chats",0, GetPanel)
end
...
sendDocument(Config.Sudoid, 0, 0, 1, nil, './Cli.lua', dl_cb, nil)
sendDocument(Config.Sudoid, 0, 0, 1, nil, './Config.lua', dl_cb, nil)
sendDocument(Config.Sudoid, 0, 0, 1, nil, './Api.lua', dl_cb, nil)
...
if tonumber(Company.sender_user_id) == tonumber(Config.TD_ID) then
...
if tonumber(user) == tonumber(Config.TD_ID) then
sendText(msg.chat_id, msg.id,  '⇜ من نمیتوانم پیام خودم را چک کنم', 'md')
return false
end
if redis:sismember('GlobalyBanned:',user) then
...
if tonumber(Company.id) == tonumber(Config.TD_ID) then
sendText(msg.chat_id, msg.id,  '⇜ من نمیتوانم پیام خودم را چک کنم', 'md')
return false
end
if Company.id then
...
if tonumber(v.user_id) == tonumber(Config.TD_ID)  then
return true
end
...
if tonumber(mutes) == tonumber(Config.TD_ID) then
sendText(msg.chat_id, msg.id,  '⇜ من نمیتوانم پیام خودم را چک کنم', 'md')
return false
end
...
if tonumber(user_id) == tonumber(Config.TD_ID) then
  sendText(msg.chat_id, msg.id, "⇜ من نمیتوانم پیام های خودم را پاک کنم", "md")
return false
    end
...
if tonumber(mutess) == tonumber(Config.TD_ID) then
...
if (matches == 'menu' or matches == 'فهرست' or matches == 'help' or matches == 'راهنما') and is_JoinChannel(msg) then
function GetPanel(MaTaDoR,Company)
if Company.results and Company.results[0] then
sendInline(msg.chat_id,msg.id, 0, 1, Company.inline_query_id,Company.results[0].id)
else
...
get(Config.BotHelper, msg.chat_id, 0, 0, msg.chat_id,0, GetPanel)
end
...
if (matches == 'filter' or matches == 'فیلتر') and is_JoinChannel(msg) then
sendText(msg.chat_id, msg.id, '⇜ `لطفا کلماتی که میخواهید فیلتر شوتد را وارد کنید`\n`توجه فقط 45 ثانیه وقت دارید`', 'md')
redis:setex("AddFilterPro:"..msg.chat_id,80,true)
end
...
if matches == 'nerkh'  or matches == 'نرخ' then
local nerkh = [[
...
sendText(msg.chat_id, msg.id, nerkh, 'md')
end
-------------------------------
end
------MaTaDoR Company---------.
...
if msg.sender_user_id == Config.TD_ID then
redis:incr("Botmsg")
