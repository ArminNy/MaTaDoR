...
local Bot_Api = 'https://api.telegram.org/bot' ..Config.token
local offset = 0 
...
local success, code, headers, status = https.request{
url = "https://api.telegram.org/file/bot" ..Config.token.. "/" .. file_info.result.file_path,
...
{
{text = '⇜ راهنمای ربات', callback_data = 'helpall:'..chat_id}
},
{
{text = '⇜ بستن فهرست مدیریتی', callback_data = 'Exit:'..chat_id}
},
{
{text="⇜ کانال تیم ما",url="https://telegram.me/"..Config.ChannelInline..""}
}
}
...
keyboard.inline_keyboard = {
{
{text = '⇜ تنظیمات', callback_data = 'Settings:'..chat_id}
},{
...
local arabic = (arabiced == "Warn") and "اخطار" or ((arabiced == "Kick") and "اخراج" or ((arabiced == "Mute") and "سکوت" or ((arabiced == "Enable") and "فعال" or "غیرفعال")))
local english = (englished == "Warn") and "اخطار" or ((englished == "Kick") and "اخراج" or ((englished == "Mute") and "سکوت" or ((englished == "Enable") and "فعال" or "غیرفعال")))
	...
	keyboard.inline_keyboard = {
		{
			...
			{text = '⇜ غیر فعال', callback_data = cb.."disable:"..chat_id}
		},
		{
...
if Company.from.id == Config.TD_ID or Company.from.id == Config.Sudoid then
if redis:get('CheckBot:'..chat_id) then
local keyboard = {}
keyboard.inline_keyboard = {{{text = '⇜ تنظیمات گروه', callback_data = 'management:'..chat_id}},{{text= '⇜ اطلاعات گروه' ,callback_data = 'groupinfo:'..chat_id}},{{text = '⇜ راهنمای ربات', callback_data = 'helpall:'..chat_id}},{{text= '⇜ بستن فهرست مدیریتی' ,callback_data = 'Exit:'..chat_id}},{{text="⇜ کانال تیم ما",url="https://telegram.me/"..Config.ChannelInline..""}}}
AnswerInline(Company.id,'settings','Group settings',chat_id,'⇜ به بخش تنظیمات و مدیریت خوشآمدید','Markdown',keyboard)
else
local keyboard = {}
keyboard.inline_keyboard = {{{text="⇜ کانال تیم ما",url="https://telegram.me/"..Config.ChannelInline..""}}}			
...
if cerner == 'helpall:'..chat_id..'' then
local keyboard = {}
keyboard.inline_keyboard = {{{text="• راهنمای سودو",callback_data = '/helpsudo:'..chat_id}},{{text="• راهنمای قفلی",callback_data = '/helplock:'..chat_id},{text="• راهنمای پاکسازی",callback_data = '/helpclean:'..chat_id}},{{text="• راهنمای تنظیمی",callback_data = '/helpset:'..chat_id},{text="• راهنمای مدیریتی",callback_data = '/helpmod:'..chat_id}},{{text= '⇜ بازگشت' ,callback_data = 'Menu:'..chat_id}}}
EditInline(msg.inline_id,'⇜ به فهرست راهنمای ربات خوشآمدید',keyboard)
end
if cerner == '/helpsudo:'..chat_id..'' then
local help =[[
⇜ راهنمای سودو های ربات‌ (دوزبانه)
...
local keyboard = {}
	keyboard.inline_keyboard = {
		{
			{text = '⇜ بازگشت', callback_data = 'helpall:'..chat_id}
		}
	}
EditInline(msg.inline_id,help,keyboard)
end
if cerner == '/helplock:'..chat_id..'' then
local help =[[
⇜ راهنمای قفل های ربات‌ (دوزبانه)
...
• [لینک | ویرایش | تگ | هشتگ | دکمه شیشه ای | سلفی | سنجاق | استیکر | فوروارد | فارسی | انگلیسی | سرویس | فونت | رسانه | عکس | آهنگ | ویس | فیلم | فایل | موقیت مکانی | گیف | بازی | مخاطب | متن | همه | ریپلای | ربات | دستورات | هرزنامه | پیام مکرر]
...
if cerner == '/helpclean:'..chat_id..'' then
local help =[[
⇜ راهنمای پاکسازی ربات‌ (دوزبانه)
...
if cerner == '/helpset:'..chat_id..'' then
local help =[[
⇜ راهنمای تنظیمی ربات‌ (دوزبانه)
...
if cerner == '/helpmod:'..chat_id..'' then
local help =[[
⇜ راهنمای مدیریتی ربات (دوزبانه)
